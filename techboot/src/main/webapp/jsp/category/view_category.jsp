...
<section class="container instructor-profile-block">
	<div class="row">
		<div class="col-xs-12 col-sm-12">
			...
			<c:if test="${not empty errorMessage}">
				...
				<aside class="bg-gray aback cover">
					...
					<form:form name="myForm" method="post" commandName="categoryValue"
						action="search-category" modelAttribute="categoryValue"
									...
									<div class="form-row">
										<div class="form-group">
											<label class="element-block fw-normal font-lato"> <form:input
													type="text" id="category" path="category"
			...
			<c:if test="${!empty catgoryList.list}">
					...
					<table class="table topics-data-table tab-full-responsive"
						...
						<thead class="bg-theme  hidden-xs">
							<tr>
								<th>S.No</th>
								<!-- <th>Created Date</th>  -->
								<th style="width: 72px;"> CategoryName</th>
						...
						<tbody>
							<c:forEach items="${catgoryList.list}"
								var="catgoryList" varStatus="status">
								<tr>
 ...
 --%>
									...
									<td data-title="categoryName"><c:out
											value="${catgoryList.category}"></c:out></td>

									<td data-title="Edit"><a
										href="edit-category.html?categoryId=${catgoryList.categoryId}">
											<i class="fa fa-edit"
											style="margin-left: 10px; color: #ffc000;"></i>
									</a></td>

									<td data-title="Delete"><a
										href="delete-category.html?categoryId=${catgoryList.categoryId}">
											<i class="fa fa-trash"
											style="margin-left: 10px; color: #ffc000;"></i>
									</a></td>
								</tr>
			...
			<nav style="text-align: center;">
				<ul class="pagination pagination-theme  no-margin center"
					...
					<c:if test="${catgoryList.currentPage gt 1}">
						<li><a href="view-category?page=1"><span><i
									class="fa fa-angle-double-left" aria-hidden="true"></i> </span></a></li>
						<li><a
							href="view-category?page=${catgoryList.currentPage - 1}"><span><i
									class="fa fa-angle-left" aria-hidden="true"></i> </span></a></li>
					</c:if>
					<c:forEach items="${catgoryList.noOfPages}" var="i">
						<c:choose>
							<c:when test="${catgoryList.currentPage == i}">
								<li class="active"><a
									style="color: #fff; background-color: #34495e">${i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="view-category?page=${i}">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if
						test="${catgoryList.currentPage lt catgoryList.totalPages}">
						<li><a
							href="view-category?page=${catgoryList.currentPage + 1}"><span><i
									class="fa fa-angle-right" aria-hidden="true"></i> </span></a></li>
						<li><a
							href="view-category?page=${catgoryList.lastRecordValue}"><span><i
