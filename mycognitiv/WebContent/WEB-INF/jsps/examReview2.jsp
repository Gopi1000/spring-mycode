...
<!-- <script type="text/javascript">
	...
	function addFun(qId1,uId1) {
		...
		$.post('AddMarkedQuestionAjax', {			
			...
			if (response == 'Success') {
				/* window.location.replace("editQuestion?quesId="+qId1); */
			} else {
				...
				<div class="row">
					<div class="col-lg-12 well">
						...
						<c:forEach var="q" items="${sessionScope.reviewQuestions}"
							varStatus="counter">
							<%-- <form action="AddMarkedQuestionAjax" method="post" >
								
								<button onclick="addFun(${q.quesId},${q.userId})">Click me</button>    
...
${counter.count}.   ${option}<br />
							...
							<font color="green">Correct Answer :
								${q.correctOption}</font>
							<br />
							<br>

							<c:if test='${q.userSelectedOption == null}'>
								<font color="#1334F1">Unanswered</font>
								<br />
							</c:if>
							<c:if test='${q.userSelectedOption != null}'>
								<font color="#1334F1">You Choosed : ${q.userSelectedOption}</font>
								<br />
							</c:if>
							<br />
							<c:if test='${q.userSelectedOption == q.correctOption}'>
						...
						<br /> <br />
						<div align="center">
							<a href="${pageContext.request.contextPath}/admin_home"><img height="50"
								width="50"
